...
class GUI:
    ...
    def build_gui(self):
        ...
        tk.Button(frame_buttons, text="Select File", command=self.load_file).pack(side='left', padx=2)
        tk.Button(frame_buttons, text="Add entry", command=self.add_entry).pack(side='left', padx=2)
        tk.Button(frame_buttons, text="Delete entry", command=self.delete_selected).pack(side='left', padx=2)
        tk.Button(frame_buttons, text="Copy Data", command=self.copy_selected).pack(side='left', padx=2)
        tk.Button(frame_buttons, text="Paste Data", command=self.paste_entries).pack(side='left', padx=2)
        tk.Button(frame_buttons, text="Process Data", command=self.process_data).pack(side='right', padx=2)
        ...
        self.data_tree.pack(padx=5, pady=5, fill='both', expand=True)

        self.result_tree = ttk.Treeview(self.root, columns=('Name', 'QEH', 'QEI', 'QEF'), show='headings')

        for col in self.result_tree['columns']:
            self.result_tree.heading(col, text=col)
        self.result_tree.pack(padx=2, pady=10, fill='both', expand=True)

        self.save_button = tk.Button(self.root, text="Save Results", command=self.save_result, state='disabled')
        self.save_button.pack(padx=2, pady=10, side='right')
    ...
    def process_data(self):
        ...
        for row in self.file_data:
            result_row = self.qepest.compute_params(QEPestInput.from_array(row[1:])).to_array()
